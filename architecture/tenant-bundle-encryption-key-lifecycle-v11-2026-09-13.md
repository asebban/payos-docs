...
**v11 (2026-09-13):** Closes gap #4 from "Gaps to close" (`edc pack`/`unpack` mutated files in place with no backup). `PayOSPacker.pack`/`unpack` (`payosv2-packer`) now accept an optional output directory, exposed as the `--outputdir` CLI flag in `Main.java`, mirroring the input tree under it instead of overwriting `--inputdir` — so a failed or interrupted run can simply be discarded (delete the incomplete output directory) and retried against a source that was never at risk. When `--outputdir` is omitted, the legacy in-place behavior is unchanged. Two things this closes beyond the literal "write elsewhere" ask: (1) the existing "skip a file that already carries a recognized magic header" (pack) / "skip a file that carries none" (unpack) logic — already in place since v6 to avoid double-processing — was extended to *copy the skipped file through unchanged* when `--outputdir` is set, otherwise the output tree would silently be missing every already-encrypted or already-plaintext file; (2) every file write, in both modes, now goes through a temp-file-then-atomic-rename (`PayOSPacker.writeAtomically`), closing the narrower residual risk that a process killed mid-write (not just mid-run) could leave one file truncated and corrupt regardless of `--outputdir`. `integrators/assembling and encrypting a bundle.md` had already assumed an `--outputdir` flag existed (it didn't) — it now does, in `pack` and `unpack` alike, not just as that doc guessed.

**v10 (2026-09-13):** Adds the operational detail v9 left implicit: a new §2 subsection, "Provisioning unpack access control (`unpackAuthorization`)", walks through creating the secret and — the part that actually matters — shows the two Vault policies side by side (editor-admin vs. client/integrator) that make `unpackAuthorization` unreachable for anyone but the editor. The mechanism itself doesn't change: `Main.requireUnpackAuthorization` only ever attempts a read and refuses on failure; every bit of the actual access restriction is a pre-existing Vault (or filesystem-provider) administration decision, not new code. Also documents the wildcard-policy pitfall and the filesystem-provider equivalent (a separate secret store rather than a denied path).

**v9 (2026-09-13):** Closes gap #3 from "Gaps to close" (`edc unpack` had no access control distinguishing "editor doing internal verification" from "anyone else"). `Main.java` (`payosv2-packer`) now requires a second, separate secret — `unpackAuthorization` by default, configurable via `--unpack-auth-secret-name`/`PAYOS_UNPACK_AUTH_SECRET_NAME` — to be readable under the same secret provider/tenant as `encryptionKey` before `unpack` proceeds; only its presence is checked, its content is never inspected. This gives the actor-model rule real technical backing rather than relying on discipline alone: an integrator's or client's Vault policy, scoped only to `read` on `encryptionKey` for the runtime's on-the-fly decrypt (§2), structurally cannot read this second secret unless the editor deliberately grants it there too — which the editor simply never does. The check only applies when the key itself is resolved from a secret provider (the recommended, key-blind models from §2); the `--key <literal>` CLI escape hatch bypasses it entirely, the same pre-existing limitation gap #7 already flags for that flag. Also fixed as part of making the gate actually effective: `Main.main()` previously caught every exception and printed a stack trace without ever exiting non-zero, so a refused unpack (or any other failure) would look like success to a calling script; it now calls `System.exit(1)` on any uncaught exception. See the updated "Gaps to close" #3 below.

**v8 (2026-09-13):** Closes v5's remaining follow-up: the periodic mask-regeneration interval (`CryptoService.REMASK_INTERVAL_SECONDS`) is no longer a hardcoded constant. It now reads the `PAYOS_CRYPTO_REMASK_INTERVAL_SECONDS` environment variable at class-load time (a positive number of seconds) and falls back to the same 5-minute default if that variable is missing, blank, non-numeric, or non-positive — logged as a warning rather than failing boot, since this is a hardening tunable, not a required secret (unlike `editor-secret-service`, there is nothing to misconfigure here that would leave `CryptoService` keyless). The parsing/validation logic is exposed as the package-private `CryptoService.resolveRemaskIntervalSeconds(String)` for unit testing, since the field itself is computed once at class load and can't be re-parameterized per test. See the updated §7 step 2 below.

**v7 (2026-09-13):** Closes the bootstrap chicken-and-egg problem that earlier versions of this document (v2–v6, §3) papered over rather than actually solved: if `payos.json` is encrypted in its entirety, `ConfigLoader` cannot read its own `editor-secret-service` block to learn how to fetch the key needed to decrypt that very file (`ConfigLoader.java:92,98,107,111,117` call `CryptoService.decryptIfEncrypted` on the raw file bytes before any JSON parsing or `${ENV_VAR}` substitution happens). Earlier versions' advice to keep `editor-secret-service`'s credentials "resolvable via environment variables (`${ENV_VAR}` placeholders) rather than needing decryption first" was itself wrong — `EnvVarResolver.resolve(...)` only runs *after* `decryptIfEncrypted`, on the already-decrypted, already-parsed JSON, so a placeholder inside an encrypted file could never be reached. `EditorEncryptionKeyInitializer` now has a real, file-independent entry point, `tryInitializeFromEnvironment()`, called by `ConfigLoader.loadServerConfig()` as the very first statement, before `payos.json` is read off disk at all — it sources the provider type/config purely from `PAYOS_EDITOR_SECRET_TYPE` / `PAYOS_EDITOR_SECRET_CFG_*` / `PAYOS_EDITOR_SECRET_ENABLED` environment variables (same generic-config mapping convention `payosv2-packer`'s own `Main.readEnvConfig` already used for `PAYOS_SECRET_CFG_*`). If it resolves the key (or explicitly opts out via `=false`), the JSON-block path is skipped entirely; if no relevant variable is set, behavior is unchanged from v6 (falls back to reading `editor-secret-service` from `PayOSConfig.settings`, which must then be in plaintext, or otherwise already-decryptable, exactly as before). See the "Grounding" bullet, §2's new subsection, and §3's corrected exclusion-list guidance below. This closes the gap for `editor-secret-service` only — the classical `secret-service` block (ordinary tenant secrets via `$Secrets`) has no equivalent fix yet and is called out as a remaining gap in §3.

**v6 (2026-09-13):** Gap #2 from "Gaps to close" (`edc` not producing `P8G2`) is now implemented — `payosv2-packer`'s `PayOSPacker.pack()` (`payosv2-packer/src/main/java/ma/s2m/PayOSPacker.java`) now always writes `P8G2` (`AES/GCM/NoPadding`, a fresh `SecureRandom` 12-byte IV per file — GCM requires a unique IV per encryption under a given key, never reused — and a 128-bit authentication tag), matching exactly what `CryptoService.decryptGcm` already expected on the read side since before this change. `unpack()` still decodes legacy `P8OS` (`AES/ECB/PKCS5Padding`) read-only, so bundles packed by older tool versions remain decodable — mirroring `CryptoService.decryptIfEncrypted`'s own dual-format support. The "Grounding" bullet below and "Gaps to close" #2 are updated accordingly; nothing else in this document changes. Two things this change does **not** touch, left open for later: the `--key`/`--generatekey` CLI still only accepts/produces a 16-character key (gap #7, unchanged), and there is still no exclusion list for `connectors/`/`.capabilities/` (gap #5, unchanged).

**v5 (2026-09-13):** Gap #1 from "Gaps to close" (split-key/XOR in-memory protection) is now implemented in `CryptoService` (`payos/src/main/java/ma/s2m/payos/security/CryptoService.java`) exactly as §7 describes: `setKey(byte[])` XORs the incoming key against a `SecureRandom` mask, zeroes the caller's array immediately, and retains `mask`/`share` each inside its own holder object (`KeyMask`/`KeyShare`) rather than as two raw fields directly on `CryptoService`; each decrypt call reconstructs the key into a fresh buffer just long enough to initialize the `Cipher`, then zeroes it in a `finally` block. Beyond what §7 originally proposed, the implementation also periodically re-randomizes the mask (`REMASK_INTERVAL_SECONDS`, currently a hardcoded 5 minutes, not yet wired into `bootstrap.json`) via a daemon background task, recomputing `share` against the unchanged real key — this bounds how long a single leaked half (mask or share alone, e.g. from a partial memory leak or a swapped-out page) stays paired with a still-valid other half, without changing the structural fact that both halves remain simultaneously reachable from the `CryptoService` singleton at any instant a full heap/object-graph snapshot is taken (see the updated §7 below for the precise claim). To keep a concurrent decrypt from ever observing a mask from one rotation combined with a share from another, `mask`/`share` are no longer two independently-volatile fields — they are paired inside one small immutable `KeyPairRef` object swapped via a single atomic reference write. §7 and "Gaps to close" below are updated accordingly; nothing else in this document changes.
...
- **Since v6, `edc` (the `payosv2-packer` module) produces `P8G2` by default.** `PayOSPacker.pack()` walks every regular file under the input directory recursively (`Files.walk(...).filter(isRegularFile)`) with zero exclusion list — `.js`, `.vue`, `.json`, anything — and encrypts each one **in place**, overwriting the original, with `Cipher.getInstance("AES/GCM/NoPadding")` and a fresh random IV per file. `unpack()` still decodes legacy `P8OS` (`Cipher.getInstance("AES")`, JCE default = AES/ECB/PKCS5Padding) read-only, for bundles produced by older tool versions. `operations/cli-tools-guide.md` and `integrators/decryption mechanism.md` previously stated `edc` already produced `P8G2` before this was true, or that it never would — both are now accurate for `pack`'s output as of this version; see the "Reconciliation" table for what in those docs may still need a pass (the exclusion list and multi-format hybrid claims are unrelated, still-open issues).
...
- **Since v7, key resolution has two entry points, tried in order, and the first one never touches a file.** `ConfigLoader.loadServerConfig()` first calls `EditorEncryptionKeyInitializer.tryInitializeFromEnvironment()` — before `payos.json` is even read off disk — which resolves (or explicitly disables) the key purely from `PAYOS_EDITOR_SECRET_TYPE` / `PAYOS_EDITOR_SECRET_CFG_*` / `PAYOS_EDITOR_SECRET_ENABLED` environment variables. Only if none of those are set does it fall back to the original `initialize(Map)` path described above (reading `editor-secret-service` from `PayOSConfig.settings`, populated from the parsed `payos.json`). This is what makes it possible for `payos.json` to be encrypted in its entirety, `editor-secret-service` block included — see §2's new subsection and §3's corrected exclusion-list guidance.
...
#### Provisioning `editor-secret-service` when `payos.json` is itself encrypted

If the editor chooses to encrypt `payos.json` in its entirety rather than carving `editor-secret-service` out of it (§3's exclusion list is then optional for this one block), the block cannot live inside `payos.json` at all — nothing can read it there before a key exists to decrypt the file. Since v7, set it instead via environment variables on the client's (or integrator's) process/container, read directly by `EditorEncryptionKeyInitializer.tryInitializeFromEnvironment()` before any file is opened:

```
PAYOS_EDITOR_SECRET_TYPE=vault
PAYOS_EDITOR_SECRET_CFG_ADDRESS=https://vault-central.s2m.internal:8200
PAYOS_EDITOR_SECRET_CFG_KV_MOUNT=client1-editor-secrets
PAYOS_EDITOR_SECRET_CFG_TOKEN=s.XXXXXXXXXXXX
```

Each `PAYOS_EDITOR_SECRET_CFG_<NAME>` variable maps to the generic config key `<name>` (uppercase-with-underscores → lowercase-with-hyphens, e.g. `KV_MOUNT` → `kv-mount`) — the same fields the JSON-block form of `editor-secret-service.configuration` would otherwise carry; see [configuration/editor-secret-service.md](../configuration/editor-secret-service.md) for the full field list per provider type. `PAYOS_EDITOR_SECRET_ENABLED=false` is the environment-variable equivalent of the JSON block's `enabled: false` opt-out.

These variables take full precedence over the JSON block: if any of `PAYOS_EDITOR_SECRET_TYPE` / `PAYOS_EDITOR_SECRET_CFG_*` / `PAYOS_EDITOR_SECRET_ENABLED` is set, `EditorEncryptionKeyInitializer` resolves (or explicitly skips) the key from the environment alone and never looks for an `editor-secret-service` block in `payos.json` at all — a mix (some fields from JSON, some from environment) is not supported, by design, to avoid ambiguity about which source wins for which field. Provisioning these variables is itself the same kind of secret-delivery problem as the key custody models in the table above — a Kubernetes Secret injected as environment variables, a systemd `EnvironmentFile`, or an orchestrator-native secrets mechanism are the usual answers, never a plaintext `.env` file checked into anything.

#### Provisioning unpack access control (`unpackAuthorization`)

Since v9, `edc unpack` (§7 explains why this is a disk-level decryption step reserved to the editor) additionally requires a second secret — `unpackAuthorization` by default — to be readable under the same secret provider/tenant as `encryptionKey` before it will proceed; only presence is checked, never content (`Main.requireUnpackAuthorization`, `payosv2-packer/src/main/java/ma/s2m/Main.java`). **This mechanism creates nothing on its own — `Main.java` only ever attempts to read the secret and refuses if that read fails.** The actual restriction is 100% a Vault-administration (or filesystem-provider) decision, made once by the editor, exactly the way `encryptionKey` custody already is above.

**Creating the secret** — same shape as `encryptionKey` (`VaultClient` stores the value under a JSON field named after the secret itself, per the per-client mount pattern above):

```bash
vault kv put client1-editor-secrets/default/unpackAuthorization unpackAuthorization="yes" type=string
```

The value (`"yes"`) is arbitrary and never inspected — only readability matters. Run this once, from the editor's own admin access, for each client mount.

**What actually restricts access — two different policies on the same Vault instance:**

```hcl
# editor-admin-policy.hcl — attached only to the editor's own build/verification credentials
path "client1-editor-secrets/data/default/encryptionKey" {
  capabilities = ["read", "create", "update"]
}
path "client1-editor-secrets/data/default/unpackAuthorization" {
  capabilities = ["read", "create", "update"]
}
```

```hcl
# client1-policy.hcl — attached to the client's (or integrator's) runtime credential, unchanged
# from §2 above: it simply never mentions unpackAuthorization.
path "client1-editor-secrets/data/default/encryptionKey" {
  capabilities = ["read"]
}
```

Vault is default-deny: a token's policy must *explicitly* grant a path before that token can touch it — omission is not a special case, it behaves identically to an explicit denial. So the client/integrator policy above was already correct and needs no change; `unpackAuthorization` is protected simply by never appearing in it. When that token's `edc unpack` tries to read `unpackAuthorization`, Vault answers `403`, `Main.requireUnpackAuthorization` catches it, and refuses to proceed.

**Pitfall:** never scope a client/integrator policy with a wildcard like `path "client1-editor-secrets/data/default/*" { capabilities = ["read"] }` — that would grant `unpackAuthorization` by accident along with `encryptionKey`. Always list exact paths, one per secret, as in the example above.

**Filesystem provider equivalent:** there is no policy system to configure — isolation instead comes from the client receiving an entirely separate `root`/`keyfile` secret store from the editor's own. The editor only ever writes `unpackAuthorization` into their *own* internal filesystem secret store; it is never copied into whatever `root`/`keyfile` gets delivered to a client or integrator, so the read simply fails with "secret not found" rather than "access denied" — same outcome, different failure mode.
...
- **`editor-secret-service`** — as of v7, this block no longer needs to be excluded at all: provision it via `PAYOS_EDITOR_SECRET_TYPE`/`PAYOS_EDITOR_SECRET_CFG_*`/`PAYOS_EDITOR_SECRET_ENABLED` environment variables instead (§2's "Provisioning `editor-secret-service`..." subsection), and `payos.json` can be encrypted in its entirety, this block included. **Correction to v2–v6 of this document:** earlier versions recommended keeping this block's credentials resolvable via `${ENV_VAR}` placeholders "rather than needing decryption first" — that was wrong. `EnvVarResolver.resolve(...)` only runs on the parsed JSON *after* `ConfigLoader` has already called `CryptoService.decryptIfEncrypted` on the file's raw bytes (`ConfigLoader.java:92,98,107,111,117`), so a placeholder sitting inside an encrypted file could never be reached — decryption would already have failed for lack of a key before any placeholder substitution runs. `${ENV_VAR}` placeholders only ever kept literal credential values out of the file's *content*; they never made the file itself encryptable. If the environment-variable path above isn't used for some reason, `editor-secret-service` must stay in a file (or part of a file) that is not itself encrypted.
- The classical **`secret-service`** block (backing `$Secrets` for ordinary tenant secrets) has **no equivalent environment-variable path today** — this remains a real, open gap, not fixed by v7 (which only addresses `editor-secret-service`). The same `EnvVarResolver`-runs-after-decrypt caveat applies to it: `${ENV_VAR}` placeholders inside it only work if the file it lives in is not itself encrypted. If it lives in a file that must be fully encrypted, keep it in a separate, unencrypted file rather than assuming placeholders solve this.
...
edc --encryption pack \
    ...
    --outputdir ./build/client1-editor-bundle-packed \
...
```bash
head -c 4 ./build/client1-editor-bundle-packed/config/bootstrap.json | xxd
# Expected: 50 38 47 32   (P 8 G 2 — as of v6, edc pack's actual output)
```

**Since v11, `--outputdir` (used above) writes the packed result to a separate tree, leaving `--inputdir` completely untouched** — a failed or interrupted run can simply be discarded and retried against a source that was never at risk; every file write is also now atomic (temp file + rename), so a process killed mid-write can't leave one file truncated either (see "Gaps to close" #4 and `cli-tools/edc.md` for the full mechanism). Without `--outputdir`, `edc` still mutates files **in place**; the old advice to always run against a disposable copy remains good defense-in-depth practice either way, but `--outputdir` is the technical fix, not just discipline.
...
**Implemented since v5 — protecting the key while it's resident in memory.** Before this, `CryptoService.setKey(byte[])` (since v3) held the raw key for the process lifetime as a single `SecretKeySpec` field — one contiguous, readable block for as long as the process ran. To raise the bar against a memory-dump/debugger attack against a running `payos-runtime` process (a real concern precisely because the whole point of on-the-fly decryption is that the process has to be able to reconstruct the key while it runs), `CryptoService` now splits the key instead of holding it whole:

1. **At startup, once (`setKey`):** receive the raw key from `EditorEncryptionKeyInitializer` (§2) — exactly as before. Generate a same-length random mask via `SecureRandom`. Compute `share = key XOR mask`. Immediately zero the caller's raw key bytes in place. Hold `mask` and `share` for the process's lifetime, each inside its own holder object (`KeyMask`, `KeyShare`) rather than as two raw byte-array fields directly on `CryptoService`, so they're not trivially found together by a single reference walk of one object's fields.
2. **Periodic mask regeneration (goes beyond the original proposal):** a daemon background task (`REMASK_SCHEDULER`) re-randomizes `mask` and recomputes `share` against the same, unchanged real key every `REMASK_INTERVAL_SECONDS` — 5 minutes by default, overridable per deployment since v8 via the `PAYOS_CRYPTO_REMASK_INTERVAL_SECONDS` environment variable (a positive number of seconds; any missing/blank/non-numeric/non-positive value falls back to the 5-minute default with a logged warning rather than failing boot). `mask` and `share` are paired inside one small immutable `KeyPairRef` object, swapped via a single atomic volatile reference write, so a decrypt call running concurrently with a rotation always reads either the fully-old pair or the fully-new one — never a mask from one rotation combined with a share from another.
3. **Per decrypt-on-the-fly call** (inside `decryptGcm`/`decryptLegacy`): reconstruct `key = mask XOR share` into a fresh, short-lived buffer, use it to initialize the `Cipher`, then zero that reconstructed buffer immediately in a `finally` block — the same "hold it only as long as the operation needs it, then zero it" discipline the codebase already applies to `SecretValue`, just applied to a value that's reconstructed on demand instead of being read fresh from the secret provider every time.
4. The raw, whole key is therefore never resident in memory as one contiguous block for longer than a single `Cipher` operation, at any point after startup — including during the (likely very frequent) steady state of a running instance decrypting many files/requests over its lifetime.

**Be honest about what this does and doesn't buy you**, the same way `integrators/decryption mechanism.md` is honest about the limits of the current simple zero-after-use scheme: splitting the key defeats a *naive* memory scan (e.g. `strings` on a core dump, or a byte-pattern scan looking for one 32-byte high-entropy blob) — each half independently is indistinguishable from random noise. It does **not** defend against an attacker who can actively instrument the running JVM (a debugger, a native memory-read primitive, a scripting-sandbox escape) and catch the reconstructed key during the brief window it exists for each `Cipher` call, and it does **not** defend against an attacker who captures a full heap/object-graph snapshot at a single instant — walking from the `CryptoService` singleton to `keyPairRef` to both `KeyMask`/`KeyShare` objects is then trivial, because the process must always be able to reconstruct the key, so both halves are always simultaneously reachable from one root object. "Two separate objects" is a logical, not a physical, separation guarantee in a managed runtime, and periodic remasking does not change this structural exposure — what periodic remasking buys is narrower and different: it bounds how long any single leaked half (captured in isolation — a partial memory leak, a swapped-out page read later, a one-off logging mistake — rather than a full simultaneous snapshot of both) stays paired with a still-valid other half. Treat all of this as a real, worthwhile hardening layer that raises the cost of an attack, not as an absolute guarantee — state that plainly to anyone relying on it, exactly as the codebase's existing security docs already do for the current mechanism.
...
1. ~~**The split-key/XOR in-memory protection from §7 doesn't exist yet.**~~ **Closed in v5** — implemented in `CryptoService` exactly as described in the updated §7 above (split-once-at-startup, atomic-swap `KeyPairRef`, reconstruct-transiently-per-call, plus periodic mask regeneration beyond the original proposal). ~~One follow-up remains open: the remask interval is a hardcoded constant.~~ **Closed in v8** — `PAYOS_CRYPTO_REMASK_INTERVAL_SECONDS` now overrides it per deployment; see §7 step 2.
2. ~~**`edc` doesn't produce `P8G2` (authenticated AES/GCM) at all.**~~ **Closed in v6** — `PayOSPacker.pack()` now writes `P8G2` (AES/GCM, fresh IV per file, 128-bit tag) by default; `unpack()` still reads legacy `P8OS` for old artifacts. Two narrower follow-ups remain open, tracked separately below: the CLI key format (#7) and the missing exclusion list (#5) are unaffected by this change.
3. ~~**`edc unpack` (disk-level decryption) has no access control distinguishing "editor doing internal verification" from "anyone else".**~~ **Closed in v9** — `unpack` now requires a second secret (`unpackAuthorization` by default) readable under the same secret provider/tenant as `encryptionKey`; an integrator's or client's Vault policy, scoped only to `encryptionKey`, cannot satisfy this unless the editor deliberately grants it there too. One acknowledged limitation remains, by design rather than oversight: the `--key <literal>` CLI flag bypasses this (and every other credential-based control) entirely, since there is no secret provider involved to check against — this is the same pre-existing weakness gap #7 already flags for that flag, not a new one.
4. ~~**`edc pack`/`unpack` mutate files in place with no backup.**~~ **Closed in v11** — `--outputdir` writes the result to a separate tree, leaving `--inputdir` untouched (retry-safe: just delete the incomplete output and re-run); every write, in both this mode and the legacy in-place default, is now atomic (temp file + rename), so a process killed mid-write can no longer leave a single file truncated either.
...
None of these block following the process above with today's `P8G2`/AES-GCM mechanism (as of v6) — they're what separates "functional" from "PCI-DSS-grade," which the code's own comments (`CryptoService.java:17-26`) already say is the target.
...
| `integrators/assembling and encrypting a bundle.md` §11.3-11.4 | Shows a single `.enc` archive artifact, which still doesn't exist — `edc` always mutates/mirrors a directory tree, never produces one combined archive file. Its `--outputdir` unpack flag guess turned out directionally right, though: as of v11, `--outputdir` is real, works for both `pack` and `unpack`, and defaults to the old in-place behavior when omitted. | Treat the single-archive claim as still aspirational; the `--outputdir` flag itself no longer is — this document's §4 and §7 (v11) are the accurate reference for its actual behavior. |
...
- `payos/src/main/java/ma/s2m/payos/security/EditorEncryptionKeyInitializer.java` — resolves the editor secret provider and the key, once, at bootstrap, and injects it into `CryptoService` via `setKey(byte[])`; since v7, `tryInitializeFromEnvironment()` is the file-independent entry point tried first.

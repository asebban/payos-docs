...
- **The key comes from a secret provider configured specifically for this purpose**, via `secretProvider.getSecret(tenantId, "encryptionKey")` inside `DefaultEditorProvider.getEditorEncryptionKey` (`ma.s2m.payos.editor`, invoked from `CryptoService.loadKey()`) — a `filesystem`/`vault` connector, but resolved from the dedicated `editor-secret-service` bootstrap.json block (`CryptoService.resolveEditorSecretProvider()`), **not** the `secret-service` block that backs the `$Secrets` binding for ordinary tenant secrets. As of v2 of this document this is a deliberate architectural correction, not an oversight — see "Changelog" above and "Key model" below.
...
- `payos/src/main/java/ma/s2m/payos/security/CryptoService.java` — `decryptIfEncrypted`, `loadKey`, `resolveEditorSecretProvider`, `resolveSecretTenantId`.
- `payos/src/main/java/ma/s2m/payos/editor/DefaultEditorProvider.java` and `payos-foundation/src/main/java/ma/s2m/payos/editor/IEditorProvider.java` — the editor-secret-service-backed key lookup `CryptoService.loadKey()` delegates to.

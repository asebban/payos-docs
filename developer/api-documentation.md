...
[cli-tools/pdoc.md](../cli-tools/pdoc.md).

## Why static generation

`pdoc` is designed for **regulated, auditable delivery**: producing API documentation must
not have side effects. It scans `api/**/*.js` for annotations and emits a spec — nothing
runs. (`pdoc` runtime-safety guarantees are detailed in the tool docs.)
...
The annotation body follows OpenAPI Operation semantics. `pdoc` assembles per-endpoint
operations into a complete OpenAPI 3.1 document, deriving paths from the application/resource
layout.
